...
const FilterSortBar = ({
  ...
  const formattedDateRange = () => {
    ...
    return `${filters.dateRange.from ? fromDate : ""} - ${
      filters.dateRange.to ? toDate : ""
    }`;
  };

  const formattedAmount = () => {
    const operator = amountOperators.find(
      (o) => o.value === filters.amount.operator
    );
  ...
  return (
    <div className="flex w-full items-center gap-2">
      {/* Active Filters */}
      <div className="flex items-center gap-2 overflow-x-auto grow">
        {activeFilterCount > 0 && (
          <>
              ...
              {filters.types.map((type) => {
                ...
                return typeInfo ? (
                  <FilterChip
                    ...
                    name={typeInfo.name}
                    onClick={() =>
                      onFilterClick("remove", "types", typeInfo.id)
                    }
              ...
              {filters.categories.slice(0, 2).map((categoryId) => {
                ...
                return category ? (
                  <FilterChip
                    ...
                    name={category.name}
                    onClick={() =>
                      onFilterClick("remove", "categories", categoryId)
                    }
                    shouldShowRemove={true}
      ...
      {(sort.direction !== defaultSort.direction ||
        ...
        <Badge
          variant="outline"
          className="cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-800 text-xs px-2 py-1 flex-shrink-0"
          onClick={onSortClick}
        >
          {sort.direction === "asc" ? (
