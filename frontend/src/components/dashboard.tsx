...
export function Dashboard({ accounts, transactions }: DashboardProps) {
  ...
  return (
    <div className="px-4 py-6 space-y-6">
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-3">
                    <div className={`w-10 h-10 ${account.color} rounded-full flex items-center justify-center`}>
                      <IconRenderer iconName={account.icon} className="w-5 h-5 text-white" />
                    </div>
                    <span className="font-medium text-slate-900 dark:text-white">{account.name}</span>
                  </div>
                  <span
                    className={`font-semibold ${account.balance >= 0 ? "text-emerald-600 dark:text-emerald-400" : "text-red-500 dark:text-red-400"}`}
                  >
      ...
      {/* Expenses This Week */}
      <div>
        <h3 className="text-lg font-semibold text-slate-900 dark:text-white mb-4">Expenses This Week</h3>
            ...
            <div className="flex items-end justify-between h-32 space-x-2">
              {weeklyExpenses.map((expense, index) => (
                <div key={index} className="flex-1 flex flex-col items-center">
                  ...
                  <span className="text-xs text-slate-600 dark:text-slate-400 mt-2 font-medium">{expense.day}</span>
            ...
            <CardContent className="p-4">
              <div className="flex items-center justify-between">
                ...
                <span className="text-lg font-semibold text-blue-600 dark:text-blue-400">
                  ${monthlyData.balance.toLocaleString("en-US", { minimumFractionDigits: 2 })}
                </span>
              </div>
            </CardContent>
          </Card>
